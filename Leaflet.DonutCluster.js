...
(function (factory, window) {
    ...
    function createDonut(points, opt, cfgFn) {
        ...
        for (var i = 0; i < count; i++) {            
            ...
            for (var si = 0; si < s.length; si++) {
                ...
                if (!sumField)
                    blocks[s[si]]++;
                else blocks[s[si]] += points[i].options[sumField][si];
                // if(points[i].location=="Toledo"){
                //     console.log(s + " " + si + " " + points[i].options[sumField][si]);
                // }
            }
        }
    ...
    L.createIcon = function (value) {
        ...
        var points = [{options:{variables:['A', 'B', 'C', 'D', 'E'], value: value}}];
        var myDonut = createDonut(points, donutOpt, function (points) {
            var style;
            if (!donutOpt.style) {
                style = defaultStyle(points)
            } else {
